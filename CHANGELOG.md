...
- Linux/macOS Gradle install switched from `apt-get install gradle` (Gradle 4.x, too old for the init script) to the vendor-supported `gradle/actions/setup-gradle@v3` action pinned at 8.10.2. Windows choco install steps now append `C:\ProgramData\chocolatey\bin` to `$GITHUB_PATH` so the surefire JVM in subsequent steps can find `mvn.cmd` / `gradle.bat`.
